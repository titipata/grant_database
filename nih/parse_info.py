...
def create_grant_info():
    ...
    path_list = glob.glob('data/project/*/*.csv', recursive=True)
    sel_name = ['APPLICATION_ID', 'ACTIVITY', 'APPLICATION_TYPE',
                'AWARD_NOTICE_DATE', 'BUDGET_START', 'BUDGET_END',
                'IC_NAME', 'ORG_NAME', 'ORG_CITY', 'ORG_STATE', 'ORG_COUNTRY',
...
def create_grant_abstract():
    ...
    path_list = glob.glob('data/abstract/*/*.csv', recursive=True)
    df_all = []
    for p in path_list:
